...
public class SearchFriendWrapper {
    ...
    public SearchFriendWrapper(View row, Context context) {
        this.row = row; // The View object that represents a single row
        this.context = context;
    }

    public void populateFrom(User r, boolean requestHeader, boolean suggestHeader, boolean searchMode) {
        userId = r.uid;
        inrequest = r.request;
        ...
        getIcon().setUser(r);
        getDivider().setVisibility(requestHeader || suggestHeader ? View.VISIBLE : View.GONE);
        if (requestHeader) {
            getDivider().setText(R.string.friend_requests);
            getDivider().setTypeface(FontsUtil.MyRiad, Typeface.BOLD);
        }
        if (suggestHeader) {
            getDivider().setText(searchMode ? R.string.found_users : R.string.friend_suggest);
            getDivider().setTypeface(FontsUtil.MyRiad, Typeface.BOLD);
        }
    }
