...
public class ContactWrapper {
		...
		public ContactWrapper(View row, Context context) {
		...
		public void populateFrom(User r, boolean isHeader) { //Associate to the item components the value from the menu bean
				ContactName rr = (ContactName) r;
				contactId = rr.contactid;
				getTitle().setText(rr.contact_name);
				getDivider().setVisibility(isHeader ? View.VISIBLE : View.GONE);
				if(isHeader) {
					getDivider().setText(rr.contact_name.charAt(0)+"");
					getDivider().setTypeface(FontsUtil.MyRiad, Typeface.BOLD);
				}
				if(rr.first_name!=null && rr.last_name!=null)
				{
					getSubTitle().setVisibility(View.VISIBLE);
					getSubTitle().setText(r.first_name + " " + r.last_name);
				}
				else
				{
